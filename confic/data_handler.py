...
def get_terms_by_category(category):
    """Получение списка терминов в указанной категории, отсортированных в алфавитном порядке."""
    terms_df  = load_data('data/terms.csv')
...
def get_strategy_examples(strategy_name):
    ...
    examples = strategies_df.loc[strategies_df['strategy_name'] == strategy_name, 'examples']
...
def get_strategy_description(strategy_name):
    ...
    strategies_df = load_data('data/strategies.csv')
    description = strategies_df.loc[strategies_df['strategy_name'] == strategy_name, 'description']
...
def get_case_studies(case_title):
    ...
    case_study = cases_df.loc[cases_df['case_title'] == case_title, ['description', 'results', 'lessons_learned']]
