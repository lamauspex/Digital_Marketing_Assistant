...
# Функции для семантического поиска, генерации ответов и проверки безопасности...
...
def semantic_search(query, data, top_k=3):
    """
    Выполняет семантический поиск, находит наиболее похожие элементы на основе входного запроса.
    ...
    if isinstance(data, pd.DataFrame):
        data = data['term'].tolist()  # Если data - DataFrame, используем только нужный столбец
    ...
    top_results = np.argpartition(-cos_sim.cpu().detach().numpy(), range(top_k))[:top_k]
